...
@section('content')
    ...
    <div class="container pt-5 mx-auto">
        ...
        <ul class="nav nav-tabs mb-3" id="ex1" role="tablist">
            ...
            <li class="nav-item" role="presentation">
                <a data-mdb-tab-init class="nav-link active" id="ex1-tab-2" href="{{ route('staff.payroll-index') }}"
                    role="tab" aria-controls="data-payroll" aria-selected="false">Rekap Payroll</a>
            </li>
            <li class="nav-item" role="presentation">
                <a data-mdb-tab-init class="nav-link" id="ex1-tab-3" href="{{ route('staff.absensi-index') }}"
                                    ...
                                    @foreach ($data as $dataPayroll)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                {{ $dataPayroll->tidak_masuk == 0 ? 'Belum ada' : $dataPayroll->tidak_masuk }}
                                            </td>
                                            <td class="text-center">
                                                {{ $dataPayroll->telat == 0 ? 'Belum ada' : $dataPayroll->telat }}</td>
                                            ...
                                            <td class="text-center">{{ formatRupiah($dataPayroll->payroll_name->salary) }}
                                            </td>
                                            <td class="text-center">
                                                <a class="btn btn-outline-danger" href="{{ route('export.pdf-one', $dataPayroll->nip_pegawai) }}">Print PDF</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
